...
- **`$search` darf NICHT in die WHERE-Klausel** (weder in `bibliothek.php` noch in `abstimmungstitel.php`). Sonst lädt der Server bei `?q=...` nur noch die Treffer, und die clientseitige Suche kann anschließend nur noch in dieser Teilmenge suchen — eine Suche nach etwas anderem findet dann nichts mehr, bis die Seite ohne `q` neu geladen wird. `$search` dient ausschließlich als Startwert für das Suchfeld. Andere serverseitige Filter (z. B. `active` in abstimmungstitel.php, `deleted_at` für Nicht-Admins) sind davon nicht betroffen, weil sie kein clientseitiges Gegenstück haben
- Der "Keine Treffer für …"-Hinweis wird deshalb ebenfalls clientseitig gesteuert (`#bib-no-hits` / `#song-no-hits`, eingeblendet von `bibFilter()`/`songFilter()`); die PHP-Zeile darunter greift nur noch bei komplett leerer Tabelle
