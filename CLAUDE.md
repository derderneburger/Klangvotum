...
- Button + Dialog nur sichtbar für `sv_can_edit_chronik()` (Admin oder `has_chronik`-Flag), zusätzlich serverseitig im POST-Handler `to_chronik` erzwungen — planer.php selbst ist nur über `sv_require_leitung()` geschützt (alle Leitungs-Rollen), die Chronik-Übernahme ist eine engere Teilmenge davon
